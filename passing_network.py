...
def draw_pass_map(ax, player_position,
    ...
    if config["plot_edges"]:
        ...
        for pair_key, row in pair_stats.iterrows():
            ...
            edge_cmap = cm.get_cmap(config["edges_cmap"])
    ...
    for player_name, row in player_stats.iterrows():
        ...
        #     "custom_blues", [light_blue, dark_blue]
        # )

        # node_color = custom_blues(norm(pass_value))
    ...
    if legend:
        ax.annotate(legend, xy=(0.01*width, 0.02*height),
                    ha="left", va="bottom", zorder=7, fontsize=10, color=config["legend_color"])
